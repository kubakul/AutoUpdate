...
xbmc.log('Imported xbmc Modules resolvers.py: %fs' % (time.time() - stime), loglevel)
import string,os
xbmc.log('Imported cookielib,string,os Modules resolvers.py: %fs' % (time.time() - stime), loglevel)
import xbmcgui, xbmcaddon, xbmcplugin
xbmc.log('Imported xbmcgui, xbmcaddon, xbmcplugin Modules resolvers.py: %fs' % (time.time() - stime), loglevel)
...
def resolve_mightyupload(url):
    ...
    try:
        ...
        addon.show_small_popup('[B][COLOR green]Mash Up: MightyUpload Resolver[/COLOR][/B]','Error, Check XBMC.log for Details',
                               5000, elogo)
        return
xbmc.log('End of resolvers.py: %fs' % (time.time() - stime), loglevel)
